...
namespace FJ{
	...
	void DrawTextWithShadow(const wchar_t* text, uint32_t x, uint32_t y, uint32_t color) {
		FUNCTIONS::DrawTextWithShadow(FJ::g_GuiComponent, mc->theMinecraft->fontRenderer, (uint32_t)&getString(text), x, y, color);
	}
	...
	void DrawTextFormatted(const char* text, int arg, int x, int y, std::uint32_t color, bool shadow = false)
	{
		char option4[0x100];
		wchar_t woption4[0x100];
		_sys_memset(option4, 0, 0x100);
		_sys_memset(woption4, 0, 0x200);
		_sys_snprintf(option4, 0x100, text, (arg));
		StringToWideCharacter(woption4, option4, strlen(option4));
		if (shadow)
		{
			DrawTextWithShadow(woption4, x, y, color);
		}
		else
		{
			DrawText(woption4, x, y, color);
		}

	}
	void DrawTextFormatted2(const char* text, int arg1, int arg2, int x, int y, std::uint32_t color, bool shadow = false)
	{
		char option4[0x100];
		wchar_t woption4[0x100];
		_sys_memset(option4, 0, 0x100);
		_sys_memset(woption4, 0, 0x200);
		_sys_snprintf(option4, 0x100, text, (arg1), (arg2));
		StringToWideCharacter(woption4, option4, strlen(option4));
		if (shadow)
		{
			DrawTextWithShadow(woption4, x, y, color);
	...
	void DrawTextWithRect(const wchar_t* text, float x, float y, uint32_t color, int* backcolor)
	{
		int width = Font_width(text);
		DrawRectangle(x - 3, y - 1, width + 5, 10, backcolor);
